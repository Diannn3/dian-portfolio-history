...
- Mathematical visuals must use the real field/equations when they claim to represent them.
  Other project/system diagrams may use restrained schematic primitives, but must be labelled as
  diagrams rather than evidence or measured geometry.
- No postprocessing, no bloom, no neon. Ink-on-paper in three dimensions.
- Compact viewports prefer flat SVG equivalents. Reduced motion may use a static single-frame
  WebGL composition when that remains cheaper and clearer than swapping the whole surface.
