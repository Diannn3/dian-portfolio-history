  ...
  return (
    <svg viewBox="0 0 520 300" className="h-full w-full" role="img" aria-label="Emergency preview: incident reports linked into a prioritised graph over a hazard field">
      ...
      <g fill="none" stroke="var(--ink)" strokeOpacity="0.16" strokeWidth="1">
        ...
        <path
          key={i}
          d={`M-20 ${70 + i * 46} C 120 ${20 + i * 46}, 300 ${140 + i * 40}, 540 ${60 + i * 44}`} />
      ...
      <g fill="var(--ink)" fillOpacity="0.85">
        {[0, 1, 2, 3].map((i) =>
        <rect key={i} x={430} y={40 + i * 14} width={70 - i * 16} height="5" />
        )}
      </g>
      <text x="430" y="30" fontFamily="Geist Mono, monospace" fontSize="9" letterSpacing="1.4" fill="var(--graphite)">
        URGENCY
      </text>
      <text x="20" y="286" fontFamily="Geist Mono, monospace" fontSize="9" letterSpacing="1.4" fill="var(--graphite)">
        MULTI-CHANNEL · DEDUP
      </text>
    </svg>);
